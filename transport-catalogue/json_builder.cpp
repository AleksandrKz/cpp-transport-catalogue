...
namespace json {
    ...
    DictItemContext Builder::StartDict()
    {
        if (step_stack_.empty()) 
            throw LogicError("Must start with builder"s);
        if (root_) 
            throw LogicError("Build finished"s);
    ...
    Builder& Builder::EndDict()
    {
        if (step_stack_.back() == Step::DICT) {
            if (dicts_open_ > 0) {
                ...
                throw LogicError("No dicts open"s);
            }
        }
        else {
            throw LogicError("Cannot close dict"s);
        }
    ...
    Builder& Builder::EndArray()
    {
        if (step_stack_.back() == Step::ARR) {
            if (arrays_open_ > 0) {
                ...
                throw LogicError("No arrays open"s);
            }
        }
        else {
            throw LogicError("Cannot close array"s);
        }
        ...
        if (step_stack_.back() == Step::DICT) {
            if (dicts_open_ > 0) {
                if (!keys_[dicts_open_ - 1]) {
                    ...
                    throw LogicError("Key already set"s);
                }
            }
            else {
                throw LogicError("No dicts open"s);
            ...
            throw LogicError("Must start a dict"s);
        }
    ...
    Node Builder::Build() const {
        if (root_) {
            ...
            throw LogicError("Must finish build"s);
        }
    }
